...
import { darkenForWhiteText } from "@/lib/utils";
...
// El teal de marca (#2BB7A6) no llega a 4.5:1 sobre blanco, así que para texto
// e iconos sobre fondos claros usamos #0F7A6C (mismo tono, oscurecido).

function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="flex items-center gap-3 font-heading text-[1.375rem] font-bold tracking-[-0.01em] text-[#0D1B3D] mb-4">
      <span className="w-1.5 h-6 rounded-full bg-[#2BB7A6]" />
      {children}
    </h2>
  );
}
...
const panelClass =
  "bg-white rounded-2xl border border-[#D7E0EA] p-6 shadow-[0_1px_2px_rgba(13,27,61,0.06),0_10px_28px_-20px_rgba(13,27,61,0.35)]";

function FactCard({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center gap-3 rounded-xl border border-[#D7E0EA] bg-white p-3.5 shadow-[0_1px_2px_rgba(13,27,61,0.05)]">
      <span className="w-9 h-9 rounded-lg bg-[#2BB7A6]/15 text-[#0F7A6C] flex items-center justify-center shrink-0">
        {icon}
      </span>
      <div className="min-w-0">
        <p className="text-[11px] uppercase tracking-[0.06em] text-[#5E7186] font-bold">{label}</p>
        <p className="text-[15px] font-bold text-[#0D1B3D] truncate">{value}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-[#EDF1F7]">
      ...
      <div className="bg-white border-b border-[#D7E0EA]">
        <div className="container mx-auto px-4 py-3 flex items-center gap-2 text-sm text-[#4E6072]">
          <Link href="/tours" className="hover:text-[#0D1B3D] transition-colors flex items-center gap-1">
            <ArrowLeft className="h-3.5 w-3.5" />
            Tours
          </Link>
          <span className="text-[#94A5B6]">/</span>
        ...
        <div className={`mb-5 sm:mb-6 ${hasGallery ? "sm:grid sm:grid-cols-2 sm:gap-8 sm:items-center" : ""}`}>
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-3">
              {primaryCategory && (
                <span
                  className="text-xs font-bold px-3 py-1.5 rounded-full text-white shadow-[0_1px_2px_rgba(13,27,61,0.18)]"
                  style={{ backgroundColor: darkenForWhiteText(primaryCategory.color ?? "#0D1B3D") }}
                >
                  {primaryCategory.name}
                </span>
              )}
              {tour.isOffer && (
                <span className="text-xs font-bold px-3 py-1.5 rounded-full bg-[#FFC97A] text-[#0D1B3D] shadow-[0_1px_2px_rgba(13,27,61,0.18)]">Oferta</span>
              )}
              <span className="flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full bg-[#E1F5F1] text-[#0F7A6C] ring-1 ring-inset ring-[#2BB7A6]/40">
                <ShieldCheck className="w-3.5 h-3.5" /> Operador verificado
              </span>
            </div>

            <h1 className="font-heading text-[1.75rem] sm:text-[2.25rem] font-bold tracking-[-0.02em] text-[#0D1B3D] leading-[1.1] mb-3.5">
              {tour.title}
            </h1>

            <div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-sm text-[#41546A]">
              {tour.rating != null && (
                <span className="flex items-center gap-1.5">
                  <Star className="h-4 w-4 fill-[#FFC97A] text-[#E0A445]" />
                  <span className="font-bold text-[#0D1B3D]">{tour.rating.toFixed(1)}</span>
                  {tour.reviewCount != null && <span>({tour.reviewCount} reseñas)</span>}
                </span>
              )}
              {tour.rating != null && <span className="w-px h-4 bg-[#C6D2E0]" aria-hidden />}
              <span className="flex items-center gap-1.5">
                <MapPin className="h-4 w-4 text-[#0F7A6C]" />
                <span className="font-medium text-[#2A3B52]">
                  {tour.destination.name}
                  {tour.destination.region && `, ${tour.destination.region}`}
                </span>
              </span>
              <span className="w-px h-4 bg-[#C6D2E0]" aria-hidden />
              <span className="flex items-center gap-1.5">
                {tour.operator.logoUrl && (
                  <span className="relative w-5 h-5 rounded-full overflow-hidden ring-1 ring-[#C6D2E0] bg-white shrink-0">
                    <Image src={tour.operator.logoUrl} alt={tour.operator.name} fill sizes="20px" className="object-contain" />
                  </span>
                )}
                <span className="font-semibold text-[#0D1B3D]">{tour.operator.name}</span>
              </span>
            </div>

            {tour.shortDescription && (
              <p className="hidden sm:block text-[15px] text-[#3D4F60] leading-relaxed mt-4 pt-4 border-t border-[#D7E0EA]">
                {tour.shortDescription}
              </p>
            )}
            ...
            {hasGallery && (
              <ul className="hidden sm:flex flex-wrap gap-x-5 gap-y-2 mt-5 pt-5 border-t border-[#D7E0EA] text-sm font-medium text-[#3D4F60]">
                {trustItems.slice(1).map((t, i) => (
                  <li key={i} className="flex items-center gap-2">
                    <span className="text-[#0F7A6C]">{t.icon}</span>
                    {t.label}
                  </li>
                ))}
              </ul>
          ...
          {hasGallery && (
            <button
              ...
              className="hidden sm:block group relative rounded-2xl overflow-hidden bg-[#E9EDF2] h-[440px] ring-1 ring-inset ring-[#0D1B3D]/10 shadow-[0_12px_32px_-18px_rgba(13,27,61,0.45)]"
            >
              <Image
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {(tour.includes.length > 0 || tour.excludes.length > 0) && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {tour.includes.length > 0 && (
                  <div className="rounded-2xl border border-[#2BB7A6]/35 bg-[#2BB7A6]/[0.07] p-5">
                    <h3 className="flex items-center gap-2 font-heading font-bold text-base text-[#0D1B3D] mb-3">
                      <CheckCircle2 className="h-[18px] w-[18px] text-[#0F7A6C]" /> Incluye
                    </h3>
                    <ul className="space-y-2.5">
                      {tour.includes.map((item, i) => (
                        <li key={i} className="flex items-start gap-2.5 text-sm text-[#3D4F60]">
                          <Check className="h-4 w-4 text-[#0F7A6C] shrink-0 mt-0.5" strokeWidth={2.5} />
                          {item}
                        </li>
                      ))}
                    </ul>
                ...
                {tour.excludes.length > 0 && (
                  <div className="rounded-2xl border border-[#D7E0EA] bg-white p-5">
                    <h3 className="flex items-center gap-2 font-heading font-bold text-base text-[#0D1B3D] mb-3">
                      <XCircle className="h-[18px] w-[18px] text-[#5E7186]" /> No incluye
                    </h3>
                    <ul className="space-y-2.5">
                      {tour.excludes.map((item, i) => (
                        <li key={i} className="flex items-start gap-2.5 text-sm text-[#55677B]">
                          <X className="h-4 w-4 text-[#94A5B6] shrink-0 mt-0.5" strokeWidth={2.5} />
                          {item}
                        </li>
                      ))}
                    </ul>
            ...
            {tour.itinerary.length > 0 && (
              <div className={panelClass}>
                <button className="flex items-center justify-between w-full text-left" onClick={() => setItineraryOpen(!itineraryOpen)}>
                  ...
                  <span className="text-[#5E7186]">{itineraryOpen ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}</span>
                </button>
                {itineraryOpen && (
                    ...
                    {tour.itinerary.map((step, i) => (
                      <div key={step.stepNumber} className="flex gap-4">
                        <div className="flex flex-col items-center">
                          ...
                          {i < tour.itinerary.length - 1 && <div className="w-0.5 flex-1 bg-[#CFD9E4] my-1.5 min-h-[24px]" />}
                        </div>
                        <div className="pb-5 pt-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2 mb-1">
                            <h3 className="font-heading font-bold text-[15px] text-[#0D1B3D]">{step.title}</h3>
                            {step.duration && (
                              <span className="text-xs font-semibold text-[#0F7A6C] bg-[#2BB7A6]/15 px-2 py-0.5 rounded-full">{step.duration}</span>
                            )}
                          </div>
                          {step.description && <p className="text-sm text-[#55677B] leading-relaxed">{step.description}</p>}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className={panelClass}>
                ...
                <div className="space-y-2">
                  {tour.faqs.map((faq, i) => {
                    ...
                    return (
                      <div key={i} className={`rounded-xl border transition-colors ${open ? "border-[#2BB7A6]/40 bg-[#2BB7A6]/[0.03]" : "border-[#D7E0EA] bg-white"}`}>
                        <button className="flex items-center justify-between w-full px-4 py-3.5 text-left" onClick={() => setOpenFaqIndex(open ? null : i)}>
                          ...
                          <ChevronDown className={`h-4 w-4 text-[#55677B] shrink-0 transition-transform ${open ? "rotate-180" : ""}`} />
                        </button>
                        {open && (
                          <div className="px-4 pb-4 text-sm text-[#55677B] leading-relaxed">{faq.answer}</div>
                        )}
                      </div>
                    );
                  })}
            ...
            {tour.reviews.length > 0 && (
              <div className={panelClass}>
                ...
                {tour.rating != null && (
                  <div className="flex items-center gap-4 mb-5 pb-5 border-b border-[#D7E0EA]">
                    ...
                    <div>
                      <div className="flex items-center gap-0.5 mb-1">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <Star key={i} className={`h-4 w-4 ${i < Math.round(tour.rating ?? 0) ? "fill-[#FFC97A] text-[#E0A445]" : "fill-[#CFD9E4] text-[#CFD9E4]"}`} />
                        ))}
                      </div>
                      <p className="text-sm text-[#55677B]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {tour.reviews.map((review) => (
                    <div key={review.id} className="rounded-xl border border-[#D7E0EA] bg-white p-4">
                      <div className="flex items-center gap-2.5 mb-2">
                        <span className="w-9 h-9 rounded-full bg-[#A8CBE6]/45 text-[#0D1B3D] text-xs font-bold flex items-center justify-center shrink-0">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="flex items-center gap-0.5">
                            {Array.from({ length: 5 }).map((_, i) => (
                              <Star key={i} className={`h-3 w-3 ${i < review.rating ? "fill-[#FFC97A] text-[#E0A445]" : "fill-[#CFD9E4] text-[#CFD9E4]"}`} />
                            ))}
                          </div>
                        </div>
                      </div>
                      {review.comment && <p className="text-sm text-[#55677B] leading-relaxed">{review.comment}</p>}
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="hidden lg:block">
            <div className="sticky top-24 space-y-4">
              <div className="rounded-2xl border border-[#D7E0EA] bg-white shadow-[0_8px_30px_rgba(13,27,61,0.08)] overflow-hidden">
                <div className="h-1.5 bg-gradient-to-r from-[#2BB7A6] to-[#0D1B3D]" />
                <div className="p-6 space-y-4">
                  <div className="flex items-end justify-between gap-3">
                    {tour.priceFrom ? (
                      <div>
                        <span className="text-xs text-[#55677B]">Desde</span>
                    ...
                    {tour.rating != null && (
                      <span className="flex items-center gap-1 bg-[#EDF1F7] ring-1 ring-inset ring-[#D7E0EA] rounded-full px-2.5 py-1 shrink-0">
                        <Star className="h-3.5 w-3.5 fill-[#FFC97A] text-[#E0A445]" />
                        <span className="text-sm font-bold text-[#0D1B3D]">{tour.rating.toFixed(1)}</span>
                      </span>
                    )}
                  </div>

                  <ul className="space-y-2 pt-4 border-t border-[#D7E0EA]">
                    {trustItems.map((t, i) => (
                      <li key={i} className="flex items-center gap-2.5 text-sm font-medium text-[#3D4F60]">
                        <span className="text-[#0F7A6C]">{t.icon}</span>
                        {t.label}
                      </li>
                    ))}
                  ...
                  <div className="space-y-2.5">
                    ...
                    <div className="space-y-1.5">
                      <Button
                        variant="outline"
                        onClick={() => setLeadFormOpen(true)}
                        className="w-full border-[#2BB7A6]/60 text-[#0F7A6C] hover:bg-[#2BB7A6]/10 font-semibold rounded-xl py-6 gap-2 text-[15px]"
                      >
                    ...
                    {builder && (
                      <Button
                        ...
                        className={`w-full rounded-xl py-6 gap-2 transition-colors ${
                          isAdded ? "border-[#2BB7A6] bg-[#2BB7A6]/10 text-[#0F7A6C]" : "border-[#2BB7A6]/60 text-[#0F7A6C] hover:bg-[#2BB7A6]/10"
                        }`}
                      >
                  ...
                  <p className="text-xs text-[#5E7186] text-center">
      ...
      <div className="lg:hidden fixed bottom-0 inset-x-0 bg-white border-t border-[#D7E0EA] pl-4 pr-[84px] py-3 flex items-center gap-3 z-40 shadow-[0_-4px_16px_rgba(13,27,61,0.08)]">
        {tour.priceFrom && (
          <div className="flex-1 min-w-0">
            <span className="text-[11px] text-[#5E7186] block leading-none mb-0.5">Desde</span>
